...
public class GorgonEnemyBehavior : MonoBehaviour
{
    ...
    [SerializeField] private Vector2 stareBoxSize = new Vector2(3f, 2f); // Size of stun effect box in front of gorgon
    [SerializeField] private Vector3 stareBoxOffset = new Vector3(1f, 0f, 0f); // Offset in front of gorgon (adjusted by facing direction)
    [SerializeField] private float petrificationDuration = 5f; // Duration to petrify targets (no damage)
    ...
    private void PlayAttackAnimation()
    {
        ...
        // Gorgon stare uses attackType 5 in the animator.
        ...
        // Fallback in case an animation event is missing from the clip.
        StartCoroutine(RangedAttackEventFallback());
    ...
    private IEnumerator RangedAttackEventFallback()
    {
        ...
        if (isAttacking && !firedStareThisAttack)
        {
    ...
    // Animation Event: call this from the gorgon stare attack clip.
    public void MedusaStareAnimationEvent()
    {
        if (firedStareThisAttack)
        {
            return;
        }

        // Create stare box in front of gorgon based on facing direction
        Vector3 boxOffset = stareBoxOffset;
        if (!isFacingRight)
        {
            boxOffset.x = -boxOffset.x; // Flip box offset based on gorgon facing direction
        }
        ...
        foreach (Collider2D hit in hitColliders)
        {
            ...
            {
                playerMovement.ApplyPetrification(petrificationDuration);
                Debug.Log($"Gorgon stare petrified player for {petrificationDuration}s");
                continue;
            }

            // Petrify attack dummy
            AttackDummy dummy = hit.GetComponent<AttackDummy>();
            if (dummy != null)
            {
                // For now, dummies can receive petrification too
                Debug.Log($"Gorgon stare hit attack dummy");
            }
        }
    ...
    private void SetupEntityToEntityCollisionIgnoring()
    {
        ...
        GorgonEnemyBehavior[] allEntities = FindObjectsByType<GorgonEnemyBehavior>(FindObjectsSortMode.None);
        
        foreach (GorgonEnemyBehavior otherEntity in allEntities)
        {
    ...
    {
        GorgonEnemyBehavior[] allEntities = FindObjectsByType<GorgonEnemyBehavior>(FindObjectsSortMode.None);
        foreach (GorgonEnemyBehavior entity in allEntities)
        {
