...
public class SkeletonArcherEnemyBehavior : MonoBehaviour
{
    ...
    private Transform pendingRangedTarget;
    ...
    private void PlayAttackAnimation()
    {
        ...
        // Archer ranged attack uses attackType 5 in the animator.
    ...
    private IEnumerator RangedAttackEventFallback()
    {
        ...
        if (isAttacking && !firedProjectileThisAttack)
        {
    ...
    // Animation Event: call this from the skeleton archer attack clip.
    public void ShootArrowAnimationEvent()
    {
        if (firedProjectileThisAttack)
        {
            return;
        }
        ...
        else if (projectileCollider is BoxCollider2D existingBox)
        {
    ...
    private void SetupEntityToEntityCollisionIgnoring()
    {
        ...
        SkeletonArcherEnemyBehavior[] allEntities = FindObjectsByType<SkeletonArcherEnemyBehavior>(FindObjectsSortMode.None);
        
        foreach (SkeletonArcherEnemyBehavior otherEntity in allEntities)
        {
    ...
    {
        SkeletonArcherEnemyBehavior[] allEntities = FindObjectsByType<SkeletonArcherEnemyBehavior>(FindObjectsSortMode.None);
        foreach (SkeletonArcherEnemyBehavior entity in allEntities)
        {
